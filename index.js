...
class Sapi {
    constructor() {
        ...
        this.getLocalSapiTokenData = async () => {
        ...
        this.setLocalSapiTokenData = (data) => AsyncStorage.setItem(this.getStorageKey(), JSON.stringify(data));
        this.isTokenValid = async () => {
    ...
    init({ sapiId, tokenOwner = "anonymous", platform = "app", requireAuth = false, }) {
        ...
        (async () => {
            ...
            if (!isTokenValid) {
                ...
                const ms = new Date(tokenData.expiresAt).getTime() -
                    Date.now() -
                    this.expirationThresholdMs;
                setTimeout(() => {
                    this.refreshToken().catch(console.error);
